...
function registerNewUser($username, $password)
{
    try {
        ...
        $statement = $connection -> prepare(
            "INSERT INTO users (username,psalt,password) VALUES ('$username','$salt','$pass')"
        );
...
function authenticateUser($username, $password)
{
    try {
        ...
        $statement = $db -> prepare(
            "SELECT password, psalt FROM users WHERE username='$username'"
        );
        ...
        if (md5($password . $row['psalt']) == $row['password']) {
            $state = true;
        } else {
            $state = false;
        }
...
function submitsongtitles($username,$title,$description,$song1,$artist1,$location1,$info1,$song2,$artist2,$location2,$info2,$song3,$artist3,$location3,$info3){
    try {
    include_once "config.php";

    
echo "Congratulations! You've submitted a playlist to Soundview!"; //what the user sees when the form is submitted
    $connection = new PDO(
        "mysql:host=".DBHOST.";dbname=".DBNAME, DBUSER, DBPASS
    );
    
    $statement = $connection -> prepare(
        "INSERT INTO songtitles (username,title,description,song1,artist1,location1,info1,song2,artist2,location2,info2,song3,artist3,location3,info3) VALUES ('$username','$title','$description','$song1','$artist1','$location1','$info1','$song2','$artist2','$location2','$info2','$song3','$artist3','$location3','$info3')");
